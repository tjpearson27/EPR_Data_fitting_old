...
def fitfunc(t, b, a1, T1, c):                        #defines the fit function  
...
bnds = [(b_lower, a1_lower, T1_lower, c_lower), (b_upper, a1_upper, T1_upper, c_upper)]

data = [('B', 'a1', 'T1', 'c')]
temp_counter = 0
temp_dep = []

createFolder('./data_fit_stretchedInvRec3/')
...
for file_ in allfiles:
    ...
    b, a1, T1, c = popt                                                                 #sets next temperature guess based on prev. temp refinement
    ...
    data.append((b_str, a1_str, T1_str, c_str))

    raw_data = [t, y, fitfunc(t, b, a1, T1, c)]                                         #writes ASCII file that can be imported to origin for data plotting
    temp_dep.append([temp, T1])
    os.chdir(os.path.abspath(os.curdir) + '\data_fit_stretchedInvRec3')
    np.savetxt('Data_and_Fit_{0}.txt'.format(file_.split('\\')[-1].replace('.dat', '')), 
                np.transpose(raw_data), delimiter = ',', fmt='%s')
    os.chdir('..')
    if show_plots == True:
        plt.plot(t, y, 'go', t, fitfunc(t, b, a1, T1, c), 'r--')                    #plots data
...
os.chdir(os.path.abspath(os.curdir) + '\data_fit_stretchedInvRec3')
np.savetxt('parameters_inv_rec_stretched.txt', data, delimiter = ',',fmt='%s')                      
np.savetxt('temp_dep_stretched.txt', temp_dep, delimiter = ',', fmt='%s')