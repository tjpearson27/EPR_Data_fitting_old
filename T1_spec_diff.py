...
#
#   TJP 10/18
#   for fitting normalized and phased inversion recovery curves to a monoexponential 
#       function accounting for spectral diffusion
...
def fitfunc(t, a1, T1, q, b):                        #defines the fit function  
...
#USER EDITS HERE
#==============================================================================================
b = 0.05                   #normalization factor best guess for lowest temp (usually 1) and bounds
b_lower = 0
b_upper = 0.1
a1 = 1                  
a1_lower = 0
...
fit_temp_dep = False
err_append = True
folder_name = 'data_fit_specDif_invRec_2'
#==============================================================================================
...
for file_ in allfiles:
    ...
    bnds = [(a1_lower, T1_lower, q_lower, b_lower), (a1_upper, T1_upper, q_upper, b_upper)]
    ...
    a1, T1, q, b = popt                                                             #sets next temperature guess based on prev. temp refinement
    ...
    if err_append == True:
        ...
        T1_str = round_sig_error(T1, T1_err, 1, paren = True)
        q_str = round_sig_error(q, q_err, 1, paren = True)       
        b_str = round_sig_error(b, b_err, 1, paren = True)                          #appends the esd to the end of the value to which it refers
        data.append((a1_str, T1_str, q_str, b_str))
    else:
    ...
    raw_data = [t, y, fitfunc(t, a1, T1, q, b)]                                     #writes ASCII file that can be imported to origin for data plotting
    temp_dep.append([temp, T1])
    ...
    os.chdir('..')
    if show_plots == True:
        plt.plot(t, y, 'go', t, fitfunc(t, a1, T1, q, b), 'r--')                    #plots data
...
os.chdir(os.path.abspath(os.curdir) + '\{0}'.format(folder_name))
np.savetxt('parameters_inv_rec_specDif.txt', data, delimiter = ',',fmt='%s')                      
np.savetxt('temp_dep_specDif.txt', temp_dep, delimiter = ',', fmt='%s')